// Generated by jextract

package com.microsoft.win32;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _TIME_DYNAMIC_ZONE_INFORMATION {
 *     LONG Bias;
 *     WCHAR StandardName[32];
 *     SYSTEMTIME StandardDate;
 *     LONG StandardBias;
 *     WCHAR DaylightName[32];
 *     SYSTEMTIME DaylightDate;
 *     LONG DaylightBias;
 *     WCHAR TimeZoneKeyName[128];
 *     BOOLEAN DynamicDaylightTimeDisabled;
 * };
 * }
 */
public class _TIME_DYNAMIC_ZONE_INFORMATION {

    static final StructLayout $struct$LAYOUT = MemoryLayout.structLayout(
        Constants$root.C_LONG$LAYOUT.withName("Bias"),
        MemoryLayout.sequenceLayout(32, Constants$root.C_SHORT$LAYOUT).withName("StandardName"),
        MemoryLayout.structLayout(
            Constants$root.C_SHORT$LAYOUT.withName("wYear"),
            Constants$root.C_SHORT$LAYOUT.withName("wMonth"),
            Constants$root.C_SHORT$LAYOUT.withName("wDayOfWeek"),
            Constants$root.C_SHORT$LAYOUT.withName("wDay"),
            Constants$root.C_SHORT$LAYOUT.withName("wHour"),
            Constants$root.C_SHORT$LAYOUT.withName("wMinute"),
            Constants$root.C_SHORT$LAYOUT.withName("wSecond"),
            Constants$root.C_SHORT$LAYOUT.withName("wMilliseconds")
        ).withName("StandardDate"),
        Constants$root.C_LONG$LAYOUT.withName("StandardBias"),
        MemoryLayout.sequenceLayout(32, Constants$root.C_SHORT$LAYOUT).withName("DaylightName"),
        MemoryLayout.structLayout(
            Constants$root.C_SHORT$LAYOUT.withName("wYear"),
            Constants$root.C_SHORT$LAYOUT.withName("wMonth"),
            Constants$root.C_SHORT$LAYOUT.withName("wDayOfWeek"),
            Constants$root.C_SHORT$LAYOUT.withName("wDay"),
            Constants$root.C_SHORT$LAYOUT.withName("wHour"),
            Constants$root.C_SHORT$LAYOUT.withName("wMinute"),
            Constants$root.C_SHORT$LAYOUT.withName("wSecond"),
            Constants$root.C_SHORT$LAYOUT.withName("wMilliseconds")
        ).withName("DaylightDate"),
        Constants$root.C_LONG$LAYOUT.withName("DaylightBias"),
        MemoryLayout.sequenceLayout(128, Constants$root.C_SHORT$LAYOUT).withName("TimeZoneKeyName"),
        Constants$root.C_CHAR$LAYOUT.withName("DynamicDaylightTimeDisabled"),
        MemoryLayout.paddingLayout(24)
    ).withName("_TIME_DYNAMIC_ZONE_INFORMATION");
    public static MemoryLayout $LAYOUT() {
        return _TIME_DYNAMIC_ZONE_INFORMATION.$struct$LAYOUT;
    }
    static final VarHandle Bias$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("Bias"));
    public static VarHandle Bias$VH() {
        return _TIME_DYNAMIC_ZONE_INFORMATION.Bias$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONG Bias;
     * }
     */
    public static int Bias$get(MemorySegment seg) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.Bias$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONG Bias;
     * }
     */
    public static void Bias$set(MemorySegment seg, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.Bias$VH.set(seg, x);
    }
    public static int Bias$get(MemorySegment seg, long index) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.Bias$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void Bias$set(MemorySegment seg, long index, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.Bias$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment StandardName$slice(MemorySegment seg) {
        return seg.asSlice(4, 64);
    }
    public static MemorySegment StandardDate$slice(MemorySegment seg) {
        return seg.asSlice(68, 16);
    }
    static final VarHandle StandardBias$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("StandardBias"));
    public static VarHandle StandardBias$VH() {
        return _TIME_DYNAMIC_ZONE_INFORMATION.StandardBias$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONG StandardBias;
     * }
     */
    public static int StandardBias$get(MemorySegment seg) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.StandardBias$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONG StandardBias;
     * }
     */
    public static void StandardBias$set(MemorySegment seg, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.StandardBias$VH.set(seg, x);
    }
    public static int StandardBias$get(MemorySegment seg, long index) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.StandardBias$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void StandardBias$set(MemorySegment seg, long index, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.StandardBias$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment DaylightName$slice(MemorySegment seg) {
        return seg.asSlice(88, 64);
    }
    public static MemorySegment DaylightDate$slice(MemorySegment seg) {
        return seg.asSlice(152, 16);
    }
    static final VarHandle DaylightBias$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("DaylightBias"));
    public static VarHandle DaylightBias$VH() {
        return _TIME_DYNAMIC_ZONE_INFORMATION.DaylightBias$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONG DaylightBias;
     * }
     */
    public static int DaylightBias$get(MemorySegment seg) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.DaylightBias$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONG DaylightBias;
     * }
     */
    public static void DaylightBias$set(MemorySegment seg, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.DaylightBias$VH.set(seg, x);
    }
    public static int DaylightBias$get(MemorySegment seg, long index) {
        return (int)_TIME_DYNAMIC_ZONE_INFORMATION.DaylightBias$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void DaylightBias$set(MemorySegment seg, long index, int x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.DaylightBias$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment TimeZoneKeyName$slice(MemorySegment seg) {
        return seg.asSlice(172, 256);
    }
    static final VarHandle DynamicDaylightTimeDisabled$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("DynamicDaylightTimeDisabled"));
    public static VarHandle DynamicDaylightTimeDisabled$VH() {
        return _TIME_DYNAMIC_ZONE_INFORMATION.DynamicDaylightTimeDisabled$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * BOOLEAN DynamicDaylightTimeDisabled;
     * }
     */
    public static byte DynamicDaylightTimeDisabled$get(MemorySegment seg) {
        return (byte)_TIME_DYNAMIC_ZONE_INFORMATION.DynamicDaylightTimeDisabled$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * BOOLEAN DynamicDaylightTimeDisabled;
     * }
     */
    public static void DynamicDaylightTimeDisabled$set(MemorySegment seg, byte x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.DynamicDaylightTimeDisabled$VH.set(seg, x);
    }
    public static byte DynamicDaylightTimeDisabled$get(MemorySegment seg, long index) {
        return (byte)_TIME_DYNAMIC_ZONE_INFORMATION.DynamicDaylightTimeDisabled$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void DynamicDaylightTimeDisabled$set(MemorySegment seg, long index, byte x) {
        _TIME_DYNAMIC_ZONE_INFORMATION.DynamicDaylightTimeDisabled$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, SegmentScope scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


