// Generated by jextract

package com.microsoft.win32;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _BIDI_REQUEST_DATA {
 *     DWORD dwReqNumber;
 *     LPWSTR pSchema;
 *     BIDI_DATA data;
 * };
 * }
 */
public class _BIDI_REQUEST_DATA {

    static final StructLayout $struct$LAYOUT = MemoryLayout.structLayout(
        Constants$root.C_LONG$LAYOUT.withName("dwReqNumber"),
        MemoryLayout.paddingLayout(32),
        Constants$root.C_POINTER$LAYOUT.withName("pSchema"),
        MemoryLayout.structLayout(
            Constants$root.C_LONG$LAYOUT.withName("dwBidiType"),
            MemoryLayout.paddingLayout(32),
            MemoryLayout.unionLayout(
                Constants$root.C_LONG$LAYOUT.withName("bData"),
                Constants$root.C_LONG$LAYOUT.withName("iData"),
                Constants$root.C_POINTER$LAYOUT.withName("sData"),
                Constants$root.C_FLOAT$LAYOUT.withName("fData"),
                MemoryLayout.structLayout(
                    Constants$root.C_LONG$LAYOUT.withName("cbBuf"),
                    MemoryLayout.paddingLayout(32),
                    Constants$root.C_POINTER$LAYOUT.withName("pData")
                ).withName("biData")
            ).withName("u")
        ).withName("data")
    ).withName("_BIDI_REQUEST_DATA");
    public static MemoryLayout $LAYOUT() {
        return _BIDI_REQUEST_DATA.$struct$LAYOUT;
    }
    static final VarHandle dwReqNumber$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("dwReqNumber"));
    public static VarHandle dwReqNumber$VH() {
        return _BIDI_REQUEST_DATA.dwReqNumber$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwReqNumber;
     * }
     */
    public static int dwReqNumber$get(MemorySegment seg) {
        return (int)_BIDI_REQUEST_DATA.dwReqNumber$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwReqNumber;
     * }
     */
    public static void dwReqNumber$set(MemorySegment seg, int x) {
        _BIDI_REQUEST_DATA.dwReqNumber$VH.set(seg, x);
    }
    public static int dwReqNumber$get(MemorySegment seg, long index) {
        return (int)_BIDI_REQUEST_DATA.dwReqNumber$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dwReqNumber$set(MemorySegment seg, long index, int x) {
        _BIDI_REQUEST_DATA.dwReqNumber$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle pSchema$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("pSchema"));
    public static VarHandle pSchema$VH() {
        return _BIDI_REQUEST_DATA.pSchema$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPWSTR pSchema;
     * }
     */
    public static MemorySegment pSchema$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)_BIDI_REQUEST_DATA.pSchema$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPWSTR pSchema;
     * }
     */
    public static void pSchema$set(MemorySegment seg, MemorySegment x) {
        _BIDI_REQUEST_DATA.pSchema$VH.set(seg, x);
    }
    public static MemorySegment pSchema$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)_BIDI_REQUEST_DATA.pSchema$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void pSchema$set(MemorySegment seg, long index, MemorySegment x) {
        _BIDI_REQUEST_DATA.pSchema$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment data$slice(MemorySegment seg) {
        return seg.asSlice(16, 24);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, SegmentScope scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


