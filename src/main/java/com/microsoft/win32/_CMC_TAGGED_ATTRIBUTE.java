// Generated by jextract

package com.microsoft.win32;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _CMC_TAGGED_ATTRIBUTE {
 *     DWORD dwBodyPartID;
 *     CRYPT_ATTRIBUTE Attribute;
 * };
 * }
 */
public class _CMC_TAGGED_ATTRIBUTE {

    static final StructLayout $struct$LAYOUT = MemoryLayout.structLayout(
        Constants$root.C_LONG$LAYOUT.withName("dwBodyPartID"),
        MemoryLayout.paddingLayout(32),
        MemoryLayout.structLayout(
            Constants$root.C_POINTER$LAYOUT.withName("pszObjId"),
            Constants$root.C_LONG$LAYOUT.withName("cValue"),
            MemoryLayout.paddingLayout(32),
            Constants$root.C_POINTER$LAYOUT.withName("rgValue")
        ).withName("Attribute")
    ).withName("_CMC_TAGGED_ATTRIBUTE");
    public static MemoryLayout $LAYOUT() {
        return _CMC_TAGGED_ATTRIBUTE.$struct$LAYOUT;
    }
    static final VarHandle dwBodyPartID$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("dwBodyPartID"));
    public static VarHandle dwBodyPartID$VH() {
        return _CMC_TAGGED_ATTRIBUTE.dwBodyPartID$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwBodyPartID;
     * }
     */
    public static int dwBodyPartID$get(MemorySegment seg) {
        return (int)_CMC_TAGGED_ATTRIBUTE.dwBodyPartID$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwBodyPartID;
     * }
     */
    public static void dwBodyPartID$set(MemorySegment seg, int x) {
        _CMC_TAGGED_ATTRIBUTE.dwBodyPartID$VH.set(seg, x);
    }
    public static int dwBodyPartID$get(MemorySegment seg, long index) {
        return (int)_CMC_TAGGED_ATTRIBUTE.dwBodyPartID$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dwBodyPartID$set(MemorySegment seg, long index, int x) {
        _CMC_TAGGED_ATTRIBUTE.dwBodyPartID$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment Attribute$slice(MemorySegment seg) {
        return seg.asSlice(8, 24);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, SegmentScope scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


