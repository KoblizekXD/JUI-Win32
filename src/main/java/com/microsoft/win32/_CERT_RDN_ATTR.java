// Generated by jextract

package com.microsoft.win32;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _CERT_RDN_ATTR {
 *     LPSTR pszObjId;
 *     DWORD dwValueType;
 *     CERT_RDN_VALUE_BLOB Value;
 * };
 * }
 */
public class _CERT_RDN_ATTR {

    static final StructLayout $struct$LAYOUT = MemoryLayout.structLayout(
        Constants$root.C_POINTER$LAYOUT.withName("pszObjId"),
        Constants$root.C_LONG$LAYOUT.withName("dwValueType"),
        MemoryLayout.paddingLayout(32),
        MemoryLayout.structLayout(
            Constants$root.C_LONG$LAYOUT.withName("cbData"),
            MemoryLayout.paddingLayout(32),
            Constants$root.C_POINTER$LAYOUT.withName("pbData")
        ).withName("Value")
    ).withName("_CERT_RDN_ATTR");
    public static MemoryLayout $LAYOUT() {
        return _CERT_RDN_ATTR.$struct$LAYOUT;
    }
    static final VarHandle pszObjId$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("pszObjId"));
    public static VarHandle pszObjId$VH() {
        return _CERT_RDN_ATTR.pszObjId$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPSTR pszObjId;
     * }
     */
    public static MemorySegment pszObjId$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)_CERT_RDN_ATTR.pszObjId$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPSTR pszObjId;
     * }
     */
    public static void pszObjId$set(MemorySegment seg, MemorySegment x) {
        _CERT_RDN_ATTR.pszObjId$VH.set(seg, x);
    }
    public static MemorySegment pszObjId$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)_CERT_RDN_ATTR.pszObjId$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void pszObjId$set(MemorySegment seg, long index, MemorySegment x) {
        _CERT_RDN_ATTR.pszObjId$VH.set(seg.asSlice(index*sizeof()), x);
    }
    static final VarHandle dwValueType$VH = $struct$LAYOUT.varHandle(MemoryLayout.PathElement.groupElement("dwValueType"));
    public static VarHandle dwValueType$VH() {
        return _CERT_RDN_ATTR.dwValueType$VH;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwValueType;
     * }
     */
    public static int dwValueType$get(MemorySegment seg) {
        return (int)_CERT_RDN_ATTR.dwValueType$VH.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwValueType;
     * }
     */
    public static void dwValueType$set(MemorySegment seg, int x) {
        _CERT_RDN_ATTR.dwValueType$VH.set(seg, x);
    }
    public static int dwValueType$get(MemorySegment seg, long index) {
        return (int)_CERT_RDN_ATTR.dwValueType$VH.get(seg.asSlice(index*sizeof()));
    }
    public static void dwValueType$set(MemorySegment seg, long index, int x) {
        _CERT_RDN_ATTR.dwValueType$VH.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment Value$slice(MemorySegment seg) {
        return seg.asSlice(16, 16);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, SegmentScope scope) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, scope); }
}


